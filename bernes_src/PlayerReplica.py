...
from bernes_src.globals_replica import passages, colors, pink_passages, before, after, mandatory_powers
from bernes_src.utils_replica import ask_question_json


# TODO: remove all ask_question_json & utils_replica
...
class Player:
    ...
    # TODO: select incrementally every instance & run algo
    def select(self, active_cards, game_state):
        """
            Choosing first out of all available characters for each node, which will be deleted from the list
        """
        ...
        if selected_character not in range(len(active_cards)):
            raise Exception('selected character not in range !')
